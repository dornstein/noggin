 ...
 *
 * `noggin` is any object implementing the engine's `Noggin` interface
 * — an in-process noggin (memory/file provider) or a `RemoteNoggin`
 * from `@noggin/rpc`. Both expose bound verb methods we call directly.
 *
...
export async function executeGesture(
  ...
  switch (gesture) {
    ...
    case 'moveUp': {
      ...
      if (!prev) return {};
      await noggin.move({ path, placement: { kind: 'before', anchor: prev.path } });
      return { movedKey: node.key };
    }
    case 'moveDown': {
      const next = nextSibling(nodes, path);
      if (!next) return {};
      await noggin.move({ path, placement: { kind: 'after', anchor: next.path } });
      return { movedKey: node.key };
    }
    case 'demote': {
      ...
      const prev = prevSibling(nodes, path);
      if (!prev) return {};
      await noggin.move({ path, placement: { kind: 'into', anchor: prev.path } });
      return { movedKey: node.key };
    }
    case 'promote': {
      ...
      if (!parent) return {};
      await noggin.move({ path, placement: { kind: 'after', anchor: parent.path } });
      return { movedKey: node.key };
    }
    case 'moveToFirst': {
      const first = firstSibling(nodes, path);
      if (!first || first.path === path) return {};
      await noggin.move({ path, placement: { kind: 'before', anchor: first.path } });
      return { movedKey: node.key };
    }
    case 'moveToLast': {
      const last = lastSibling(nodes, path);
      if (!last || last.path === path) return {};
      await noggin.move({ path, placement: { kind: 'after', anchor: last.path } });
      return { movedKey: node.key };
    }

    case 'toggleDone':
      if (node.done) await noggin.edit({ path, done: false });
      else await noggin.done({ path });
      return {};

    case 'delete':
      await noggin.delete({ path, recursive: hasChildren });
      return {};
